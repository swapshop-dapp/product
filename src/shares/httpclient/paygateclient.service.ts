...
import {
    SupportCryptoCurrencyEnum,
    SupportFiatCurrencyEnum
...
export default class PaygateClientService extends BaseClientService {
    ...
    async getExchangeRates(
        ...
        try {
            let data = await this.cacheManager.get<Record<string, number>>(`rates:${dto.listingCurrency}`)
            if (!_.isEmpty(data)) {
                return data
            }
            ...
            await this.cacheManager.set(`rates:${dto.listingCurrency}`, data, {ttl: 60 * 60})
            return data
        } catch (e) {
